...
public struct QuoteService {
    ...
    public func findIdx(_ input: Quote) -> Int {
        let idx = quotes.firstIndex { quote -> Bool in
            return quote.quote == input.quote && quote.author == input.author
        }

        return idx ?? -1
    }

    public func getSequentialQuote(isForward: Bool, input: Quote? = nil) -> Quote {
        ...
        // randomise if we don't have index within range
